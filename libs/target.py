...
from skyfield.api import Loader, wgs84, Star, utc
...
#        return points
    ...
    def create_track(self, offsets=False):
        ...
        run_time = arange(now, now + .25, .000347222)
        ...
        ra, dec, dist = self.astrometric.radec()
    ...
    def generate_report(self):
        ...
        for idx, item in enumerate(self.t_list):
            if self.alt_list[idx] < 10.0:
                ...
                curr_time = self.t_list[idx].utc_strftime('%Y:%m:%d:%H:%M:%S')
                if self.rise_time is None:
                    self.rise_time = curr_time
                self.fade_time = curr_time
                self.az_out = "{:.4f}".format(self.az_list[idx])
                self.alt_out = "{:.4f}".format(self.alt_list[idx])
                output = (curr_time + ", " + self.az_out +
                          ", " + self.alt_out + "\n")
    ...
    def alt_az_to_ra_dec(self, alt, az, t):
        ''' Given Elevation (alt), azimuth (az) and a datetime (t) of interest,
            find the RA Dec being observed.
        '''
        Ra_Dec = self.ant.at(t).from_altaz(alt_degrees=alt, az_degrees=az)
        return Ra_Dec
...
if __name__ == "__main__":
    test_star_trk = Target()
    test_star_trk.ready_antenna(64.97347, 147.50183, 403.6)
    test_star_trk.select_target('CAS_A')
    test_star_trk.create_track()
    test_star_trk.generate_report()
#    print(''.join(test_star_trk.report))
    t = test_star_trk.t_list[1]
    alt = test_star_trk.alt_list[1]
    az = test_star_trk.az_list[1]
    print(test_star_trk.alt_az_to_ra_dec(alt, az, t).radec())
    t = test_star_trk.t_list[-1]
    alt = test_star_trk.alt_list[-1]
    az = test_star_trk.az_list[-1]
    print(test_star_trk.alt_az_to_ra_dec(alt, az, t).radec())
